...
- Several prompt templates still reference legacy relationship aliases while the target relationship contract uses `entity_graph`.
...
Violations / drift:
- legacy relationship aliases still leak into prompt templates and downstream consumers
...
- `system_prompt.yaml`, `database_context.yaml`, `rag_context.yaml`, `agent_context.yaml`, and `text_to_sql.yaml` still reference legacy relationship aliases.
...
- remove direct reliance on legacy relationship aliases in the target contract
